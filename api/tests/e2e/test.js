...
it('should select a rating and submit it', async function () {
    ...
    // Find the select element for rating
    ...
    // Find and click the submit rating button
    const submitRatingButton = await driver.findElement(By.css('.select-rating-container button'));
    await submitRatingButton.click();
  
    // Wait for the rating to be submitted (you may need to adjust the selector based on your UI)
    // await driver.wait(until.elementLocated(By.xpath('//div[contains(text(), "Thank you for your rating!")]')), 5000);
  });
